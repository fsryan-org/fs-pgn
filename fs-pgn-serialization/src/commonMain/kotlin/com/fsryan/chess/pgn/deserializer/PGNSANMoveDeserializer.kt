...
internal fun BufferedSource.deserializeSANMove(
    position: Int,
    moveIsBlack: Boolean
): PGNDeserializationResult<PGNSANMove> {
    if (exhausted()) {
    ...
    try {
        peek().use { peekable ->
            ...
            val result = when {
                ...
                firstChar.isCastleMarker -> peekable.castle(position, charsRead = 1, moveIsBlack = moveIsBlack)
                else -> throw PGNSANIllegalCharacterException(position, firstChar)
            }
            incrementByUTF8CharacterCount(result.charactersRead)
            return result
        }
...
private fun BufferedSource.continueWithSuffixAnnotationSymbol(
    ...
    val nextChar = readUTF8Char()
    return when {
        nextChar.isSuffixAnnotationSymbol -> PGNDeserializationResult(
...
private fun BufferedSource.castle(startPosition: Int, charsRead: Int, moveIsBlack: Boolean): PGNDeserializationResult<PGNSANMove> {
